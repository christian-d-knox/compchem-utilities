...
# Last major commit to the project was 2025-11-26 (previously 2025-10-28)
# Last minor commit to the project was 2025-12-3

# Imports the various libraries needed for all functions()
import os, argparse, glob, regex #, sys # Only necessary for occasional troubleshooting
...
class Defaults:
    ...
    stalkDuration = 120
    stalkFrequency = 3
    # Job submission related. May require edits across HPC types.
...
fileNames, fullPaths, totalJobList, totalOutputs = [], [], [], []
isStalking, isLooping, isCheck, isNBO = False, False, False, False
isCustomTarget, canBench = True, True
indexOverride = 0
# Useful for single point creation and program identification
methodLine, fullMethodLine, methodList, targetProgram = [], [], [], []
fileExtension = ""
stalkingSet = set()
...
def firstTimeSetup():
    ...
    with open(os.path.join(Defaults.binDirectory, "hpc.type"), "w") as hpcFile:
        match systemType:
            case "H2P":
                hpcFile.write("H2P")
                Defaults.hpcType, Defaults.submissionList = "H2P", H2PSubmission.submissionList
            case "Bridges2":
                hpcFile.write("Bridges2")
                Defaults.hpcType, Defaults.submissionList = "Bridges2", Bridges2Submission.submissionList
            ...
            case "Stampede3":
                ...
                Defaults.hpcType, Defaults.submissionList = "Stampede3", Stampede3Submission.submissionList
                Defaults.partition = "icx"
                Defaults.CPU = 80
                Defaults.memoryRatio, Defaults.memoryBuffer, Defaults.highMemoryRatio = 200/80, 0, 200/80
            case _:
...
if os.path.isfile(os.path.join(Defaults.binDirectory, "hpc.type")):
    with open(os.path.join(Defaults.binDirectory, "hpc.type"), "r") as hpcFile:
        ...
        match hpcLine:
            case "H2P":
                Defaults.hpcType, Defaults.submissionList = "H2P", H2PSubmission.submissionList
                Defaults.cluster, Defaults.partition = "smp", "pliu"
            case "Bridges2":
                Defaults.hpcType, Defaults.submissionList = "Bridges2", Bridges2Submission.submissionList
                Defaults.partition = "RM-shared"
                Defaults.memoryRatio, Defaults.memoryBuffer, Defaults.highMemoryRatio = 2, 0, 2
...
def getCoords(fileName, outputFileName):
    ...
    at, X, Y, Z = regExMapper("                         Standard orientation:                         ",
                              True,"readFromEnd","coordinates",fileName)
...
def gaussianChargeFinder(geometryFile):
    chargeLine = regExMapper("Charge",False,"readFromStart","charge",geometryFile)
    ...
    if chargeSub[2] == '':
        del chargeSub[2]
    charge, multiplicity = chargeSub[2], chargeSub[5]
    ...
    match molecule.extensionType:
        case Defaults.gaussianExtension:
            ...
            with open(inputFile, 'w') as jobInput:
                ...
                jobCPU, jobMem = str(Defaults.CPU), str(Defaults.CPU * Defaults.memoryRatio)
...
def genReRun(molecule,skipIndex):
    originalMethod = regExMapper("Will use up to",False,"readFromEnd","method",molecule.fullPath)
...
def jobStalking(jobSet, duration, frequency):
    ...
    # obsessiveQueuev2
    command, finishedJobs, pingTime = ["squeue -h --me --format='%25j %10T %18R %S %20M'"], [], time.time()
                    ...
                    for job in jobSet:
                        if job[0] == result[index].split()[0]:
                            if os.path.isfile(job[1]) and os.path.getsize(job[1]) > 0:
                                containsStability = regExMapper("Stability analysis",False,"readFromEnd",
                                                                "contains",job[1])
                                if containsStability is not None:
                                    hasStabilized = regExMapper("The wavefunction is already stable.",True,
                                                                "readFromEnd","contains",job[1])
                                    ...
                                    stabilityInsert = ""
                                convergeCriteria = regExMapper("         Item               Value     Threshold  Converged?",
                                                               True,"readFromEnd","convergence",job[1])
                                if convergeCriteria is not None:
                                    cprint("Job " + str(result[index].split()[0]) + " is currently running, and "
                                                                                    "has converged on " + str(
                                        convergeCriteria) + " out of 4 criteria.\n    "
                                           + stabilityInsert + " Current duration is " + str(result[index].split()[4]),
                                           "light_magenta")
                                else:
                                    cprint(
                                        "Job " + str(result[index].split()[0]) + " is currently running. Convergence "
        ...
        for job in jobCopy:
            ...
            if job[0] in stalkStatus and os.path.isfile(job[1]) and os.path.getsize(job[1]) > 0:
                for termination in Defaults.terminationVariants:
                    termLine = regExMapper(termination,True,None,"contains",job[1])
...
def goodVibesProcessor(inputFile):
    goodVibesData = regExMapper("Structure",False,"readFromStart","goodVibes",inputFile)
    dataFrame = pandas.DataFrame(goodVibesData)
...
# Refactord the code to do all memory-mapped regex searches through one function to reduce copypasta
def regExMapper(rawString,isReverseSearch,readType,returnType,inFile):
    # Memory mapped searches must be done in binary mode
    byteString = rawString.encode()
    with open(inFile, 'r') as inputFile:
        # Immediately close the file, but map it to memory. Allows byte-wise searching with no buffer size hassle
        with closing(mmap(inputFile.fileno(), 0, access=ACCESS_READ)) as data:
            if isReverseSearch:
                regExSearch = regex.search(byteString, data, regex.IGNORECASE.REVERSE)
            else:
                regExSearch = regex.search(byteString, data, regex.IGNORECASE)
            if regExSearch is not None:
                # Governs if further I/O is done where the search match started, or ended
                match readType:
                    case "readFromStart":
                            pointer = regExSearch.starts()
                    case "readFromEnd":
                            pointer = regExSearch.ends()
                    case _:
                        return None
                data.seek(pointer[0])
                # Determines the exact data returned from the function. This is copypasta from the original functions that
                # were consolidated
                match returnType:
                    case "coordinates":
                        data.read(2)
